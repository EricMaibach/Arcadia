...
def load_create_app_description() -> str:
    """Load the create_app tool description from the shared markdown file."""
    try:
        # Try multiple possible locations for the description file
        possible_paths = [
            # If running from mcpserver directory
            Path(__file__).parent.parent / "shared" / "create_app_description.md",
            # If running from project root
            Path.cwd() / "shared" / "create_app_description.md",
            # If running from mcpserver directory (cwd-based)
            Path.cwd().parent / "shared" / "create_app_description.md",
        ]
        
        for description_path in possible_paths:
            if description_path.exists():
                logger.info(f"Found description file at {description_path}")
                return description_path.read_text(encoding='utf-8')
        
        # If not found, log all attempted paths for debugging
        logger.warning(f"Could not find description file. Searched in: {[str(p) for p in possible_paths]}")
        return "Create a new WASM application by submitting Rust code that implements the ArcadiaApp trait"
    except Exception as e:
        logger.warning(f"Error reading description file: {e}, using default description")
        return "Create a new WASM application by submitting Rust code that implements the ArcadiaApp trait"
...
async def handle_list_tools() -> List[types.Tool]:
    ...
    return [
        ...
        types.Tool(
            name="create_app",
            title="Create App",
            description=load_create_app_description(),
            inputSchema={
                "type": "object",
                "properties": {
                    "appId": {
